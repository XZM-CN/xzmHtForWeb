! function(O, Q, s) {
	...
	var h = "ht",
		...
		m = f.widget,
		S = r.widgetBorder,
		X = r.widgetBackground,
		p = r.widgetIconBorder,
		T = r.transparent,
		Y = r.background,
		U = r.highlight,
		x = f.Default,
		...
		J = x.labelFont,
		_ = x.labelColor,
		n = x.labelSelectColor,
		y = x.widgetIndent,
		l = x.startDragging,
		g = x.createElement,
		N = x.getClientPoint,
		u = x.scrollBarInteractiveSize,
		K = x.getInternal(),
		F = (K.createView, K.createDiv),
		W = K.createCanvas,
		d = K.createImage,
		B = K.setCanvas,
		R = K.initContext,
		a = K.translateAndScale,
		w = K.isString,
		Z = K.drawBorder,
		t = K.getImageWidth,
		e = K.getImageHeight,
	...
	c(yr, Q, {
		...
		validateImpl: function() {
			var T = this,
				...
				o = T.getWidth(),
				b = T.getHeight(),
				p = T._pressed || T._selected,
				J = p ? T._selectBackground : T._background;
			B(V, o, b);
			var X = R(V);
			...
			var n = T._iconColor,
				...
				P = T._label,
				C = T.getLabelFont(),
				E = p ? T._labelSelectColor : T._labelColor,
				F = null == P ? Gf : G(C, P),
				z = F.width,
				l = F.height;
	...
	c(ud, Q, {
		...
		validateImpl: function() {
			...
			var c = w._padding,
				W = w._iconColor,
				u = H(U ? w.getCheckIcon() : w.getRadioIcon()),
				i = t(u),
				A = e(u);
	...
	c(Dn, Q, {
		...
		setValue: function(F) {
			var c = this.slider,
				L = c.getPadding() + c.getButtonWidth() / 2,
				g = c.getMin();
		...
		var i = this,
			N = i._view = K.createView(null, i),
			t = i._listView = new m.ListView,
			L = i._canvas = W(N),
			Y = t._view,
			d = t.sm();
		...
		toIcon: function(v) {
			var g = this,
				a = g._values,
				l = g._icons;
			if(l && a && l.length === a.length)
				for(var h = 0; h < a.length; h++)
					if(g.isEqual(a[h], v)) return l[h];
			return null
		},
		...
		open: function() {
			...
			if(!a.isOpened()) {
				...
				var g = o(),
					...
					v = g.top,
					n = g.height,
					Z = a._view.getBoundingClientRect(),
					y = Z.left + Y,
					b = Z.top + v,
					u = Z.height,
					T = a.getIndent(),
		...
		setInputValue: function(x) {
			var D = this,
				...
				W = x.value;
			if(k)
				for(var b = 0; b < k.length; b++) W === k[b] && (W = S[b]);
		...
		Q.setHeight($), Q.setWidth(80), I.style.display = "inline-block", K.setBorder(I, x.comboBoxBorderColor), I.addEventListener("keydown", function(s) {
			...
			if(j(v)) {
				var r = v.target,
					o = Q._dropDownComponentInstanceView;
		...
		open: function() {
			var h = this;
			if(!h.isOpened()) {
				var e = x.getClass(h._dropDownComponent),
					u = h._dropDownComponentInstance = new e(h),
					N = h._dropDownComponentInstanceView = u.getView(),
					R = o(),
					P = R.left,
					g = R.top,
					G = R.width,
					r = R.height,
					m = h._view.getBoundingClientRect(),
					C = m.left + P,
					c = m.top + g,
					D = (m.width, m.height),
					a = h.getIndent(),
		...
		close: function(B) {
			...
			if(t.isOpened()) {
				...
				var U = t._dropDownComponentInstance,
					W = t._dropDownComponentInstanceView;
	...
	m.ColorPicker = function() {
		...
		var E = this,
			H = "px",
			n = E._view = K.createView(null, E),
			P = E.$10o = document.createElement("div"),
			U = E.$11o = F(),
			v = E.$12o = F(),
			Q = P.style;
		...
		$5o: function(o, _, G) {
			var T, Q, Z = Math.max(Math.max(o, _), G),
				e = Math.min(Math.min(o, _), G),
				O = Z;
			if(e == Z) T = 0, Q = 0;
			else {
				var S = Z - e;
		...
		$3o: function(v, C, G) {
			...
			var M, j, L;
			if(3 == v.length) M = v.substr(0, 1), M += M, j = v.substr(1, 1), j += j, L = v.substr(2, 1), L += L;
			else {
		...
		$9o: function() {
			var n = this._view,
				z = Ur(n, "input.color_R").value,
				T = Ur(n, "input.color_G").value,
				N = Ur(n, "input.color_B").value,
				r = Ur(n, "input.color_A").value || 1;
		...
		onOpened: function(Y) {
			if(Y) {
				...
				if(j.$11o.style.backgroundColor = Y, 0 === Y.indexOf("#") ? (Y = Y.substr(1, 6), c = j.$3o(Y, !0), c = [c.r, c.g, c.b]) : 0 === Y.indexOf("rgb") && (Y = Y.substring(Y.indexOf("(") + 1, Y.indexOf(")")), c = Y.split(",")), c) {
					var r = 4 === c.length ? c[3] : 1,
						M = j.$5o(c[0] / 255, c[1] / 255, c[2] / 255),
						a = Ur(i, ".satval_pointer");
	...
	c(Nb, Q, {
		...
		addRow: function(L, O, m, Z) {
			...
			if(L) {
				for(var i = 0; i < L.length; i++) {
					var A = L[i];
					A && ((A.tagName || A.getView) && (L[i] = {
						element: A
					}), K.initItemElement(A), null != A.id && (g._itemMap[A.id] = A))
				}
		...
		validateImpl: function() {
			...
			for(T = 0; k > T; T++) {
				...
				var d = H + u;
				if(c) {
					var q = V.getRowBorderColor(r),
						e = V.getRowBackground(r);
		...
		validateRow: function(_, p, F, r, q, h, e, X) {
			...
			for(0 > D && (B -= D, D = 0), s = 0; W > s; s++) {
				if($ = F[s], Y = r[s], w(Y) ? (m = Y.split("+"), k = parseFloat(m[0]), Y = k > 1 ? k : k / G * D, k = parseFloat(m[1]), Y += k > 1 ? k : k / G * D) : 1 >= Y && (Y = Y / G * D), $) {
					if(_) {
						var c = f.getItemBorderColor($),
							M = f.getItemBackground($);
